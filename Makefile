...
default: 
	...
	@echo "Choices: setup-h, setup-l, 141-h, 141-l, images, list (prints copy-paste select image creation), counterr, toperr, typeerr, allerr"
...
Lab-setup-141.xml:
	git diff-index --name-only master | grep Lab-setup-141.xml && git diff-index --stat master

141-Lab-Manual.xml:
	git diff-index --name-only master | grep 141-Lab-Manual.xml && git diff-index --stat master
...
setup-h: ${BEE}/user/mathbook-setup-html.xsl Lab-setup-141.xml 
	xsltproc ${BEE}/user/mathbook-setup-html.xsl Lab-setup-141.xml

setup-l: ${BEE}/user/mathbook-setup-latex.xsl Lab-setup-141.xml 
	xsltproc ${BEE}/user/mathbook-setup-latex.xsl Lab-setup-141.xml
...
141-h: ${BEE}/user/mathbook-lab-html.xsl 141-Lab-Manual.xml
	xsltproc ${BEE}/user/mathbook-lab-html.xsl 141-Lab-Manual.xml
	@echo ""

141-l: ${BEE}/user/mathbook-lab-latex.xsl 141-Lab-Manual.xml
	xsltproc ${BEE}/user/mathbook-lab-latex.xsl 141-Lab-Manual.xml
	sed -i.sedfix -f 141-Lab-Manual.sed fall-lab-manual.tex

fall-lab-manual.tex: 141-l

141-p: fall-lab-manual.tex
...
checksize:
	...
	@grep "PDF version" 141-Lab-Manual.xml | sed 's#.*\">\(.*\)\.pdf (\([0-9][0-9]*\) kB.*#echo "echo -e \\\"claim: \2\\\\tactual:\\\" \\\\`expr `stat --printf=%s \1.pdf` / 1000\\\\`\\\\\t\1" >> list.two#g' >> list.one 
	./list.one 
	./list.two
...
fixsize:
	...
	@grep "PDF version" 141-Lab-Manual.xml | sed 's#.*\">\(.*\)\.pdf (\([0-9][0-9]*\) kB.*#echo "echo \\"s^\1.pdf (\2 kB)^\1.pdf (\\\\`expr `stat --printf=%s \1.pdf` / 1000\\\\` kB)^g\\"" >> step.two#g' >> step.one 
	./step.one 
	./step.two > step.sed
	sed -i.size -f step.sed 141-Lab-Manual.xml

fall-lab-manual.pdf: 141-p
...
images: 141-Lab-Manual.xml Lab-setup-141.xml
	${BEE}/script/mbx -v -c latex-image -f svg -d images ${AIY}/141-Lab-Manual.xml
#	${BEE}/script/mbx -v -c latex-image -r [specific image reference] -f svg -d images ${AIY}/141-Lab-Manual.xml
	${BEE}/script/mbx -v -c latex-image -f svg -d images ${AIY}/Lab-setup-141.xml
...
list: 141-Lab-Manual.xml Lab-setup-141.xml
	cat Lab-setup-141.xml | \
		sed 's/^ *<image/<image/g' | \
		grep '<image' | grep -v "images" | \
		sed 's/ width=.*>/>/g' | \
		sed 's+^.*xml:id=\"\(.*\)\">+ls images/\1.svg || C:/Users/tensen/Desktop/Book/mathbook/script/mbx \-v \-c latex-image \-r \1 \-f svg \-d images ${AIY}/Lab-setup-141.xml+g'
	@echo "*************************"
	cat 141-Lab-Manual.xml | \
		sed 's/^ *<image/<image/g' | \
		grep '<image' | grep -v "images" | \
		sed 's/ width=.*>/>/g' | \
		sed 's+^.*xml:id=\"\(.*\)\">+ls images/\1.svg || C:/Users/tensen/Desktop/Book/mathbook/script/mbx \-v \-c latex-image \-r \1 \-f svg \-d images ${AIY}/141-Lab-Manual.xml+g'

counterr: ${BEE}/../jing-trang/build/jing.jar ${BEE}/schema/pretext.rng 141-Lab-Manual.xml  Lab-setup-141.xml
	@echo `java -jar ${BEE}/../jing-trang/build/jing.jar ${BEE}/schema/pretext.rng Lab-setup-141.xml | wc -l`" errors"
	@echo `java -jar ${BEE}/../jing-trang/build/jing.jar ${BEE}/schema/pretext.rng 141-Lab-Manual.xml | wc -l`" errors"

toperr: ${BEE}/../jing-trang/build/jing.jar ${BEE}/schema/pretext.rng 141-Lab-Manual.xml  Lab-setup-141.xml
	java -jar ${BEE}/../jing-trang/build/jing.jar ${BEE}/schema/pretext.rng Lab-setup-141.xml | head -5
	@echo "*************************"
	java -jar ${BEE}/../jing-trang/build/jing.jar ${BEE}/schema/pretext.rng 141-Lab-Manual.xml | head -5

typeerr: ${BEE}/../jing-trang/build/jing.jar ${BEE}/schema/pretext.rng 141-Lab-Manual.xml  Lab-setup-141.xml
	java -jar ${BEE}/../jing-trang/build/jing.jar ${BEE}/schema/pretext.rng Lab-setup-141.xml | \
		sed 's/.*:\([0-9][0-9]*\):\([0-9][0-9]*\): error: element "\([a-zA-Z][a-zA-Z]*\)".*/\3 line \1:\2/g' | \
		sort -k1
	@echo "*************************"
	java -jar ${BEE}/../jing-trang/build/jing.jar ${BEE}/schema/pretext.rng 141-Lab-Manual.xml | \
		sed 's/.*:\([0-9][0-9]*\):\([0-9][0-9]*\): error: element "\([a-zA-Z][a-zA-Z]*\)".*/\3 line \1:\2/g' | \
		sort -k1

# To find the errors on "todo"  (must change in two places)                                                vvvv                                                 vvvv
# 	java -jar ${BEE}/../jing-trang/build/jing.jar ${BEE}/schema/pretext.rng 141-Lab-Manual.xml | grep ": element \"todo" | sed 's/.*:\([0-9][0-9]*\):\([0-9][0-9]*\):.*/todo line \1:\2/g'
#                                                                                                          ^^^^                                                 ^^^^

allerr: ${BEE}/../jing-trang/build/jing.jar ${BEE}/schema/pretext.rng 141-Lab-Manual.xml  Lab-setup-141.xml
	java -jar ${BEE}/../jing-trang/build/jing.jar ${BEE}/schema/pretext.rng Lab-setup-141.xml | \
		sort -k4  
	@echo "*************************"
	java -jar ${BEE}/../jing-trang/build/jing.jar ${BEE}/schema/pretext.rng 141-Lab-Manual.xml | \
		grep -v `grep -n "known tag abuse 1" 141-Lab-Manual.xml | sed 's/:.*//g'` | \
		sort -k4  

all: setup-h 141-h setup-l 141-l images
